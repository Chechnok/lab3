...
def sign_certificate(ca_key, ca_cert, public_key,
                     common_name):  # даною функцією створюється сертифікат для клієнта/сервера, які підписуються СА
    subject = x509.Name([
        x509.NameAttribute(NameOID.COUNTRY_NAME, "UA"),
...
CA_CERT_FILE = "ca_cert.pem"  # Константа для сертифіката CA
...
def run_client():
    ...
    print("Перевірка сертифіката CA...")
    if not os.path.exists(CA_CERT_FILE):
        print("Сертифікат CA не знайдено. Завантаження обов'язкового сертифіката...")
        time.sleep(1)  # Імітація часу створення сертифіката
    print("Запускаємо клієнт...")
    client()


if __name__ == "__main__":
    # Створення процесу для сервера
    server_process = multiprocessing.Process(target=run_server)

    # Запуск процесу сервера
    server_process.start()

    # Невелика затримка перед стартом клієнта, щоб сервер встиг запуститись
    time.sleep(2)

    # Запуск клієнта у поточному процесі
    run_client()
